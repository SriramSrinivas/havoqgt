...
TEST(test_static_bfs, test_static_bfs) {
  ...
  havoqgt::bias_randwalk(graph, bfs_level_data, edge_data, vertex_data);
...
//  for(size_t i=1; i<num_vertices; ++i) {
    ...
    //if(vertex.owner() == mpi_rank) {
       ...
       //EXPECT_LE(edge_data[eitr], 100);
       //EXPECT_LE(vertex_data[vertex], 100);
    //}
   }
  } 
  for (auto vitr = graph->vertices_begin(); vitr != graph->vertices_end(); ++vitr) {
    for(auto eitr = graph->edges_begin(*vitr); eitr != graph->edges_end(*vitr); ++eitr) {
      ...
      std::cout << graph->locator_to_label(eitr.target()) << " " << edge_data[eitr] << " " 
                << graph->locator_to_label(*vitr) << vertex_data[*vitr] << std::endl;
  }
 }
}
