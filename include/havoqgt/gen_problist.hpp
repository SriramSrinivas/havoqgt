...
class gen_problist_visitor {
public:
  typedef typename Graph::vertex_locator                 vertex_locator;
  gen_problist_visitor() { }

  //template<typename MTData>
  gen_problist_visitor(vertex_locator _vertex, vertex_locator _from, MTData _meta_data)
    : vertex(_vertex)
    , from(_from)
    , meta_data(_meta_data) { }

  gen_problist_visitor(vertex_locator _vertex)
    : vertex(_vertex)
    , from(_vertex) {}
  //  , meta_data(0) { }
  ...
  bool init_visit(Graph& g, VisitorQueueHandle vis_queue, AlgData& alg_data) const {
	 // for(auto vitr = g.vertices_begin(); vitr != g.vertices_end(); ++vitr) {
		...
		//gen_problist_visitor new_visitor(eitr.target(), *vitr, std::get<0>(alg_data)[*vitr] );
		if( eitr.source() == vertex) {
		gen_problist_visitor new_visitor(eitr.target(), vertex, std::get<0>(alg_data)[vertex] );
  ...
  bool visit(Graph& g, VisitorQueueHandle vis_queue, AlgData& alg_data) const {
      ...
      for(auto eitr = g.edges_begin(vertex); eitr != g.edges_end(vertex); ++eitr) {
        ...
        if( g.locator_to_label(from) == g.locator_to_label(neighbor)) {
          std::get<0>(alg_data)[eitr] = meta_data;
  ...
  friend inline bool operator>(const gen_problist_visitor& v1, const gen_problist_visitor& v2) {
    return false;
  }
...
void gen_problist(TGraph* g, LevelData& level_data, EdgeData& edge_data) {

  typedef  gen_problist_visitor<TGraph, uint16_t>    visitor_type;
