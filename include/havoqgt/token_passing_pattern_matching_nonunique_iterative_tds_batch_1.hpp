#ifndef HAVOQGT_TOKEN_PASSING_PATTERN_MATCHING_NONUNIQUE_ITERATIVE_TDS_BATCH_1_HPP_INCLUDED 
#define HAVOQGT_TOKEN_PASSING_PATTERN_MATCHING_NONUNIQUE_ITERATIVE_TDS_BATCH_1_HPP_INCLUDED
...
//  auto vq = create_visitor_queue<visitor_type, /*havoqgt::detail::visitor_priority_queue*/tppm_queue>(g, alg_data);
  ...
  uint64_t max_ranks_per_itr = mpi_size / 24; //(mpi_size / 36) / 4;
...
///////////////////////////////////////////////////////////////////////////////
  ...
  for (auto superstep = 0; superstep <= max_itr_count /*<=pattern_cycle_length*/; superstep++) {
    ...
    if (mpi_rank == 0) {
      std::cout << "Token Passing [" << pl << "] | Batch #" << batch_count << " | Superstep #" << superstep;
    }       
...
//    }
  ...
  if (mpi_rank == 0) {
    std::cout << "Token Passing [" << pl << "] | Batch #" << batch_count 
      << " | Global Pattern Count : " << batch_global_path_count << std::endl;
  }
  // Test
...
#endif //HAVOQGT_TOKEN_PASSING_PATTERN_MATCHING_NONUNIQUE_ITERATIVE_TDS_BATCH_1_HPP_INCLUDED
