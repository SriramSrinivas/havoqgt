...
int main(int argc, char** argv) {
  ...
  graph_type::vertex_data<double, std::allocator<double> >  next_rank(*graph);
  MPI_Barrier(MPI_COMM_WORLD);
  double time_start = MPI_Wtime();
  page_rank(*graph, cur_rank, next_rank, true);
  MPI_Barrier(MPI_COMM_WORLD);
  double time_end = MPI_Wtime();
  if(mpi_rank == 0) {
    std::cout << "PageRank Time = " << time_end - time_start << std::endl;
  }
  
  // for(auto itr = graph->vertices_begin(); itr != graph->vertices_end(); ++itr) {
  //   std::cout << cur_rank[*itr] << std::endl;
  // }
