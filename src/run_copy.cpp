...
int main(int argc, char** argv) {
  ...
  // BFS Experiments
  {
    ...
    graph_type::edge_data<uint16_t, std::allocator<uint16_t> >   copy_edge_data(*graph);
//********************** Jiyuan: Add a new edge_data type for copy_edge function
    MPI_Barrier(MPI_COMM_WORLD);
    if (mpi_rank == 0) {
...
//**************** Jiyuan: run bfs first, get bfs_level_data result, then call copy_edge function. Edge result is stored in copy_edge_data
      havoqgt::breadth_first_search(graph, bfs_level_data, bfs_parent_data,
          source); 
      std::cout<< "bfs done" <<std::endl;
      havoqgt::copy_edge(graph, bfs_level_data, copy_edge_data);
