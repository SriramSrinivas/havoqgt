...
print("sampling", str(r1), "random walkers")

os.chdir(os.path.dirname('/g/g17/osimpson/evolving/havoqgt/build/catalyst.llnl.gov/'))
os.system( 'srun -N32 --ntasks-per-node=24 --distribution=block ./src/run_random_walk_simulation -i /l/ssd/output -m /l/ssd/metadata -c 10000 -n ' + str(r1) + ' -a brad_pitt_id.txt -s ' + str(start_time) + ' -e ' + str(end_time) + ' -r output -d 15 -t ' + str(r2) )
